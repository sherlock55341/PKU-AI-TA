...
`uv sync` installs runtime and dev dependencies. Use `list-assignments` to discover assignment `gradeBookPK` values before grading or submission. `review --auto-approve-safe --all` is the fastest way to approve all non-flagged submissions before manual review. The `grade`, `review`, and `submit` commands map directly to the production workflow described in `README.md`. Run `uv run pytest` before opening a PR.
